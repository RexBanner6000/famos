...
parser.add_argument('--workers', type=int, help='number of data loading workers', default=0)  # 0 - single main process
parser.add_argument('--outputFolder', default='.', help='folder to output images and model checkpoints')
parser.add_argument('--trainOverfit',
                    type=bool,
                    default=False,
                    help='always use same image and same templates -- better in sample, worse out of sample'
                    )
...
parser.add_argument('--multiScale',
                    type=bool,
                    default=False,
                    help='multi-scales of mixing features; if False only full resolution; if True all levels'
                    )
...
parser.add_argument('--skipConnections',
                    type=bool,
                    default=True,
                    help='skip connections in  Unet -- allows better content reconstruct'
                    )
parser.add_argument('--Ubottleneck',
                    type=int,
                    default=-1,
                    help='Unet bottleneck, leave negative for default wide bottleneck'
                    )
...
parser.add_argument('--fEntropy',
                    type=float,
                    default=.5,
                    help='regularization weight of entropy -- forcing low entropy results in 0/1 values in mix tensor A'
                    )
...
nDep = opt.nDep
# noise added to the deterministic content mosaic modules -- in some cases it makes a difference,
# other times can be ignored
